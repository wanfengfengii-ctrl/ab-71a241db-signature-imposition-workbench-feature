...
- 可勾选**自动混合 8/16/32 页容量**：勾选后每帖页数成为容量上限，可另录
  **不可拆页段**（闭区间，如 `16-17,33-40`，重叠/相邻自动合并）；成功后
  逐帖显示规划容量、补白与约束命中，顶层显示逐帖容量序列；
...
自动模式勾选后追加 `"auto_mode": true`，填了不可拆页段时再追加
`"unbreakable_segments": "16-17,33-40"`；取消勾选时这两个字段都不发送，
请求与响应恢复为固定模式结构。
